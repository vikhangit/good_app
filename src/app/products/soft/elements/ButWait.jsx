...
function Icon({ open }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`${open ? "rotate-90" : ""} w-5 h-5 text-indigo-600 mr-3.5`} strokeWidth={4} >
...
export default function ButWait() {
    ...
    useEffect(() => {
        ...
        FaqsData.map((item, index) => {
            if (index % 2 === 0) {
                arr1.push({ ...item, show: false })
            } else {
                arr2.push({ ...item, show: false })
            }
        })
        setData2([
            {
    ...
    const handleOpen = (indexParent, indexChild) => {
        data2[indexParent].data[indexChild].show = !data2[indexParent].data[indexChild].show
        setData2([...data2])
    }
    return (
        <div className='bg-indigo-600 pt-20 pb-14 px-24'>

            <h1 className='text-3xl font-semibold text-center text-white'>But wait! {"There's"} more ...</h1>
            <div className='grid grid-cols-2 items-start gap-x-10 mt-10'>
                {
                    data2.map((item, index) =>
                        <div key={index}>
                            {
                                item.data.map((itemChild, indexChild) => <Accordion
                                    key={indexChild}
                                    open={itemChild.show}
                                    icon={
                                        <Icon
                                            open={indexChild.show}
                                        />}
                                >
                                    <div

                                        className="border border-blue-gray-200 mb-4 rounded-sm bg-white"
                                    >
                                        <AccordionHeader
                                            onClick={() => { handleOpen(index, indexChild) }}
                                            className="border-0 py-5 text-xl font-semibold flex-row-reverse justify-end"
                                        >
                                            {itemChild.name}
                                        </AccordionHeader>
                                        <AccordionBody>
                                            <div
                                                className='text-base font-light px-12 leading-7 [&>p>a]:text-indigo-600 [&>p>a]:font-medium [&>p>a:hover]:underline [&>ul]:list-disc [&>ul]:pl-4 [&>ul>li]:mt-4'
                                                dangerouslySetInnerHTML={{ __html: itemChild.desc }}>
            ...
            <div className='w-2/3 mx-auto mt-20 flex flex-col items-center'>
                <h2 className='text-xl font-medium text-center'>
                    CMS Hub has everything you need to build a powerful, optimized website that converts.
                </h2>
                 <Link
                          href="#"
                          className="text-sm font-medium bg-white py-3.5 px-5 rounded-sm hover:opacity-60 mt-4"
                      >
                          See CMS Hub plans and pricing
                      </Link>
            </div>
        </div>
    )
